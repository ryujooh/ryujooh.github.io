---
title: "서울시 부동산 실거래가 탐색적 데이터 분석 (EDA)"
date: "2026-06-09"
category: "데이터분석"
tags: ["pandas", "EDA", "시각화", "아파트실거래가"]
---

# 🏢 서울시 부동산 실거래가 탐색적 데이터 분석 (EDA) - 자치구 중심 분석

본 프로젝트는 서울시 부동산 실거래가 데이터를 바탕으로 사전 데이터 전처리를 거쳐, 자치구별 시장 동향, 가격 수준 및 양극화 현상, 물리적 요인과 가격의 교차 분석, 그리고 계약 취소 및 직거래 등의 특이사항 분포를 입체적으로 분석한 결과 리포트입니다.
...
1. **유효 거래 필터링**: '취소일'이 존재하는 행(Row)을 제외하여 실제 계약이 완료된 거래 데이터만 추출하였습니다. (단, 계약 취소율 분석 시에는 전체 계약 건수를 분모로 활용)
2. **파생 변수(Derived Variable) 생성**:
...
3. **이상치 정제**: 극단적인 고가/저가 특수 거래가 전체 통계를 왜곡하는 것을 방지하기 위해, IQR(Interquartile Range) 기준에 맞춰 1㎡당 단가의 이상치를 필터링하였습니다.

---

## 📊 시나리오 A. 자치구별 시장 동향 및 거래량 분석
> **질문**: 서울시 25개 자치구 중 거래가 활발한 지역은 어디이며, 시간에 따라 거래량은 어떤 흐름을 보이는가?

### ① 자치구별 누적 거래량 비교
전체 기간 동안 어느 구에서 가장 거래가 활발하게 일어났는지 누적 거래 건수를 비교하였습니다.
...
### ③ 자치구별 건물 용도 비중
각 자치구 내에서 주거 상품 형태(아파트, 오피스텔, 연립다세대, 단독다가구 등)의 거래 비율이 어떻게 구성되어 있는지 살펴보았습니다.

![자치구별 건물 용도 비중](../images/A3_gu_building_usage.png)

> **인사이트 💡**
> 도봉구, 노원구 등 전통적 주거 베드타운 지역은 **아파트** 거래 비중이 압도적으로 높게 나타납니다. 반면, 강남권 및 서초권 등 상업/오피스 지구가 밀집된 지역에서는 **오피스텔** 및 **연립다세대**의 거래 비중이 상대적으로 높게 분포하여 지역별 주거 형태 특성이 뚜렷하게 관측됩니다.

---

## 📊 시나리오 B. 자치구별 가격 수준 및 양극화 분석
> **질문**: 자치구 간의 부동산 가격 격차는 어느 정도이며, 구 내부의 가격 편차는 어떠한가?

### ① 자치구별 평균 단가 비교
면적의 영향을 차단한 '1㎡당 평균 단가'를 구하여 서울시 자치구의 실질적인 가격 순위를 도출했습니다.

![자치구별 평균 단가 비교](../images/B1_gu_average_unit_price.png)

> **인사이트 💡**
> 부동산 단가 분석 결과, **강남구**가 가장 높은 평균 가격(평당 약 7,714만 원)을 형성하며 독보적인 1위를 차지했습니다. 서초구, 용산구, 송파구가 그 뒤를 이어 상위 그룹을 형성하고 있으며, 최상위 지역과 최하위 지역 간의 단가 차이가 무려 3~4배 이상 벌어지는 심각한 지역 양극화 현상이 입증되었습니다.

---

### ② 자치구별 가격 분포 현황 (Box Plot)
각 구별로 실거래가 단가가 얼마나 넓게 퍼져 있는지(편차)와 비정상적으로 높게 거래된 이상치(Outlier) 현황을 상자 수염 그림으로 나타냈습니다.
...
> **인사이트 💡**
> - **군집 0 (중저가 주거 지역군)**: 평균 단가가 비교적 저렴하고 고르게 분포하는 지역군입니다.
> - **군집 1 (하이엔드/고가 선도 지역군)**: 강남, 서초 등 최고 단가를 형성하며 취소율 및 거래 특성이 일반 자치구와 구별되는 그룹입니다.
> - **군집 2 (대단지 활성 거래 지역군)**: 노원, 송파 등 총 거래량이 월등히 많고 주거 단지 거래 회전율이 높은 지역군입니다.

---

## 📊 시나리오 C. 물리적 요인과 자치구 특성의 교차 분석
> **질문**: 면적, 연식, 층수 등의 건물 특성이 자치구별로 가격에 미치는 영향이 다른가?
...
> **인사이트 💡**
> 일반적으로는 신축 건물의 가격이 높아야 하지만, 일부 자치구에서는 **30년 초과 노후 건물의 평당 단가가 신축을 상회하거나 비등한 역전 현상**이 나타납니다. 특히 강남, 서초, 송파 등 재건축 정비사업에 대한 기대감이 큰 핵심 지역에서 이러한 재건축 프리미엄이 강하게 작동하는 것으로 확인됩니다.
...
> **인사이트 💡**
> 서울시 자치구의 평균 계약 취소율은 **5.70%** 수준으로 나타났으며, 특히 **중구(8.18%)**를 필두로 강남권 및 도심권 일부 지역에서 취소 비율이 높게 집계되었습니다. 단가가 높고 시장 관심도가 큰 지역일수록 호가 띄우기를 위한 자전거래 목적 혹은 계약 조건 변경에 따른 거래 취소 건의 모니터링 필요성이 제기됩니다.
...
서울시 부동산 실거래가 17만여 건에 대한 자치구 중심의 다각적 EDA를 통해 도출된 핵심 결론은 다음과 같습니다.
...
3. **재건축 프리미엄과 주거 특성**: 노원구와 같은 거래 활성화 주거 단지는 대중적인 아파트 중심인 반면, 정비사업의 기대가 큰 지역(강남, 서초 등)은 구축 연식이 되려 신축을 넘어서는 비정형적 단가 흐름을 보여 특수 목적의 투자 수요가 강하게 지배하는 성향을 보입니다.
